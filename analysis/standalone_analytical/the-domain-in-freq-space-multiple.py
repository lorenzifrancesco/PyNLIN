...
# Plot only a reasonable number of nearby shifted cubes.
index_values = range(-2, 3)


# ============================================================
# Find the vertices of one shifted cube clipped by
#
# |nu1 - nu2 + nu3| <= B/2
# ============================================================
...
def clipped_region_vertices(n1, n2, n3, spacing_ratio):
    ...
    # Every polyhedron vertex is obtained by intersecting
...
# ============================================================
# Extract true polyhedron edges
#
# scipy.spatial.ConvexHull triangulates polygonal faces.
# The function below removes artificial diagonals connecting
# coplanar triangles.
# ============================================================
...
def true_hull_edges(vertices):
    ...
    for edge, adjacent_faces in edge_to_faces.items():
        ...
        # Keep the edge only when the two adjacent triangles
        # belong to different geometric faces.
        if abs(normal_1 @ normal_2) < 1 - 1e-7:
            true_edges.append(edge)

    return hull, true_edges
...
# ============================================================
# Intersection between one shifted cube and
#
# nu1 - nu2 + nu3 = slice_value
# ============================================================

def shifted_cube_section(
    n1,
    n2,
    n3,
    spacing_ratio,
    ...
    for i, p in enumerate(cube_vertices):
        for j, q in enumerate(cube_vertices):
            ...
            # Two cube vertices are connected when they differ
            # in exactly one coordinate.
            number_of_different_coordinates = np.count_nonzero(
                np.abs(p - q) > tol
            )

            if number_of_different_coordinates == 1:
    ...
    for p, q in cube_edges:
        ...
        # Entire cube edge lies in the slicing plane.
        if abs(fp) < tol and abs(fq) < tol:
            intersection_points.extend([p, q])
            continue

        # Plane intersects the cube edge.
    ...
    # Build a two-dimensional coordinate system intrinsic
    # to the slicing plane.
    ...
    polygon_hull = ConvexHull(projected_points)

    # Return vertices ordered around the polygon boundary.
    return points[polygon_hull.vertices]


# ============================================================
# nu1 and nu3 are user-controlled; nu2 is derived from the
# frequency-matching condition nu1-nu2+nu3=nu4, with nu4 fixed by
# the requested output frequency (slice_value). Rendered as a 2D
# WDM comb via plot_wdm_quadruplet.draw_wdm_quadruplet.
# ============================================================
...
def build_figure(
    ...
    # --------------------------------------------------------
    # Generate all nonempty nearby regions
    # --------------------------------------------------------
    ...
    # One color for each channel-index family.
    q_colors = {
        -6: "#2d004b",
        -5: "#3f007d",
        -4: "#5e3c99",
    ...
    # --------------------------------------------------------
    # Add regions, edges, slices and slice boundaries
    # --------------------------------------------------------
    ...
    for region in regions:
        ...
        edge_x, edge_y, edge_z = edges_as_lines(
            vertices,
            edges,
        )
        ...
        # ----------------------------------------------------
        # Translucent admissible polyhedron
        # ----------------------------------------------------
        ...
        fig.add_trace(
            go.Mesh3d(
                ...
                # Only one legend entry for each q.
        ...
        number_of_boundary_vertices = len(section)
        ...
        triangle_j = np.arange(
            1,
            number_of_boundary_vertices + 1,
        )
        ...
        # Thick boundary of the red slice.
        closed_section = np.vstack([
            section,
            section[0],
        ])
    ...
    plot_limit = max(abs(v) for v in index_values) + 0.65 / Omega_0

    q_text = ", ".join(
        f"{q:+d}"
        for q in sorted(active_q_values)
    )

    fig.update_layout(
        font=dict(size=16),
        title=(
            f"Ω₀/B={spacing_ratio:.2f}, "
            f"ν₄/Ω₀={s_norm:+.3f}"
            f"<br><sup>"
            f"{len(regions)} admissible regions; "
        ...
        scene=dict(
            ...
            # this string never changes (set here too, not just via the
            # callback's update_layout, so the very first render already
            # carries it and doesn't cause a spurious reset on first input).
            uirevision="domain-camera",
            xaxis=dict(
                title="ν₁/Ω₀",
        ...
        legend=dict(
            ...
            # Clicking one q entry hides all traces in that
            # legend group:
            #
            # - translucent regions;
            # - black edges;
            # - red slices;
            # - red slice boundaries.
...
app.layout = html.Div(
    [
        html.H3(
            "Shifted frequency-domain regions",
        ...
        html.Div(
            [
                _slider_block(
                    ...
                    step=0.05,
                    value=1.10,
                    marks={0.55: "0.55", 1.10: "1.10", 2.20: "2.20"},
                ),
                _slider_block(
                    "ν₄/Ω₀ (output)",
                    "slice-slider",
            ...
            [
                ...
                html.Div(
                    [
                        ...
                        html.Img(
                            id="quadruplet-image",
                            src=quadruplet_image_src(1.0, -1.0, 1.10, 0.0),
                            style={"maxWidth": "100%"},
                        ),
                    ],
