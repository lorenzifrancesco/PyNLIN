...
def _dataset_path(pulse_shape: str) -> Path:
    return OUT_RESULTS / (
        f"xhkm_sum_ref_curve_{pulse_shape}_demo_h{H_MAX}_r{R_MAX}"
        f"_marg{PARTIAL_COLLISIONS_MARGIN}.npz"
    )


def _plot_stem(stem: str) -> str:
    return f"{stem}_h{H_MAX}_r{R_MAX}_marg{PARTIAL_COLLISIONS_MARGIN}"
...
def _cases() -> list[dict[str, object]]:
    cases: list[dict[str, object]] = [
        ...
        {
            "key": "rc02",
            "pulse_shape": "raised_cosine_rolloff0p2",
        ...
        {
            "key": "rc05",
            "pulse_shape": "raised_cosine_rolloff0p5",
            "pulse": RaisedCosinePulse(
                baud_rate=BAUD_RATE,
                num_symbols=NUM_SYMBOLS,
                samples_per_symbol=SAMPLES_PER_SYMBOL,
        ...
        {
            "key": "rrc02",
            "pulse_shape": "root_raised_cosine_rolloff0p2",
        ...
        {
            "key": "rrc05",
            "pulse_shape": "root_raised_cosine_rolloff0p5",
            "pulse": RootRaisedCosinePulse(
                baud_rate=BAUD_RATE,
                num_symbols=NUM_SYMBOLS,
                samples_per_symbol=SAMPLES_PER_SYMBOL,
...
def compute_case(case: dict[str, object], *, recompute: bool = False) -> Path:
...
def _load(path: Path) -> np.lib.npyio.NpzFile:
    return np.load(path)
...
def plot_ratio(paths: dict[str, Path]) -> None:
    ...
    ax.legend(fontsize=8)
    _annotate(ax)
    _finish(fig, _plot_stem("xhkm_n2_over_n1_gaussian_nyquist"))
...
def main() -> None:
    ...
    paths: dict[str, Path] = {}
    for case in cases:
        path = compute_case(case, recompute=True)
        paths[str(case["key"])] = path
        print(f"saved {path}")

    plot_single(paths["gaussian"], "Gaussian, flat profile", "xhkm_n1_n2_gaussian")
    plot_single(paths["nyquist"], "Nyquist, flat profile", "xhkm_n1_n2_nyquist")
    plot_decomposition(paths["gaussian"], "Gaussian decomposition", "xhkm_collision_decomposition_gaussian")
    plot_decomposition(paths["nyquist"], "Nyquist decomposition", "xhkm_collision_decomposition_nyquist")
    plot_compare(
        [("Gaussian", paths["gaussian"], "tab:purple"), ("Nyquist", paths["nyquist"], "tab:blue")],
        "xhkm_n1_n2_2pc_gaussian_nyquist_combined",
        include_2pc=True,
    )
    plot_compare(
        [("Gaussian", paths["gaussian"], "tab:purple"), ("Nyquist", paths["nyquist"], "tab:blue")],
        "xhkm_n1_n2_gaussian_nyquist_combined",
        include_2pc=False,
        legend_font=7,
    )
    plot_ratio(paths)
    plot_compare(
        [("Nyquist", paths["nyquist"], "tab:blue"), (r"RC $\rho=0.2$", paths["rc02"], "tab:orange")],
        "xhkm_n1_n2_2pc_nyquist_vs_raised_cosine_rolloff0p2",
    )
    plot_compare(
        [("Nyquist", paths["nyquist"], "tab:blue"), (r"RC $\rho=0.5$", paths["rc05"], "tab:green")],
    ...
    plot_compare(
        ...
        "xhkm_n1_n2_2pc_nyquist_vs_rrc_rolloff",
        legend_font=5.5,
    )
    for ld_ratio in (1, 5, 10):
        plot_single(
            paths[f"nyquist_LD{ld_ratio}"],
            rf"Nyquist, flat profile, $L/L_D={ld_ratio}$",
            f"xhkm_n1_n2_2pc_nyquist_LD{ld_ratio}",
        )
    plot_compare(
        [("No disp.", paths["nyquist"], "tab:orange"), (r"$L/L_D=10$", paths["nyquist_LD10"], "tab:blue")],
    ...
    plot_compare(
        ...
        "xhkm_n1_n2_2pc_nyquist_no_disp_vs_LD1_vs_LD10",
        legend_font=5.7,
    )
    plot_compare(
        [
        ...
        "xhkm_n1_n2_2pc_nyquist_no_disp_vs_LD1_vs_LD5_vs_LD10",
        legend_font=5.4,
    )
