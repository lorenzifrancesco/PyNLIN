...
OUT_MEDIA = Path("media/n-PC/pulse-window-convergence")
OUT_RESULTS = Path("results/pulse-window-convergence")
FIBER_LENGTH = 400.0
BAUD_RATE = 10e9
...
def _pulse(pulse_shape: str, num_symbols: int):
...
def _dataset_path(
    ...
    pts = f"{min_pts_per_collision:g}".replace(".", "p")
    return OUT_RESULTS / (
        f"xhkm_pulse_window_convergence_{pulse_shape}_{_window_token(window)}"
        f"_nsym{num_symbols}_pts{pts}_maxz{max_z_points}"
        f"_llw{float(llw_grid[0]):g}-{float(llw_grid[-1]):g}_n{llw_grid.size}.npz"
    )


def compute_num_symbols(
    *,
    pulse_shape: str,
    window: tuple[int, int, int],
    num_symbols: int,
    ...
    path = _dataset_path(pulse_shape, window, num_symbols, min_pts_per_collision, max_z_points, llw_grid)
    ...
    for llw in llw_grid:
        ...
        print(
            f"{pulse_shape} {_window_label(window)} num_symbols={num_symbols}: "
            f"L/LW={llw:.3g}, n_m={len(m_values)}, n_z={result.metadata['n_z']}, "
            f"walkoff/window={result.metadata['walkoff_fraction']:.3f}"
        )
    ...
    saved = save_xhkm_sum_reference_curves(
        ...
        baud_rate=BAUD_RATE,
        pulse_shape=f"{pulse_shape}_nsym{num_symbols}",
        mode="perfect",
        gvda=0.0,
        gvdb=0.0,
        h_values=h_values,
    ...
    np.savez(
        saved,
        **payload,
        num_symbols=np.array(int(num_symbols)),
        samples_per_symbol=np.array(int(SAMPLES_PER_SYMBOL)),
...
def _load_curves(paths: dict[int, Path]) -> dict[int, dict[str, np.ndarray]]:
    curves = {}
    for num_symbols, path in paths.items():
        with np.load(path) as data:
            curves[num_symbols] = {key: np.asarray(data[key]) for key in data.files}
    return curves


def plot_absolute(curves: dict[int, dict[str, np.ndarray]], out_dir: Path, pulse_shape: str) -> Path:
    ...
    for ax, (component, title) in zip(axes.ravel(), PLOT_COMPONENTS, strict=True):
        for num_symbols, data in curves.items():
            ax.plot(data["llw_grid"], data[component], marker="o", ms=2.5, lw=1.0, label=f"{num_symbols} symbols")
        ax.set_xscale("log")
        ax.set_yscale("log")
        ax.set_title(title)
    ...
    fig.suptitle(f"{pulse_shape.title()} xhkm pulse-window convergence")
    fig.tight_layout()
    path = out_dir / f"xhkm_pulse_window_convergence_absolute_{pulse_shape}.pdf"
    fig.savefig(path, dpi=300)
    plt.close(fig)
    return path


def plot_relative_error(curves: dict[int, dict[str, np.ndarray]], out_dir: Path, pulse_shape: str) -> Path:
    reference_num_symbols = list(curves)[-1]
    reference = curves[reference_num_symbols]
    num_symbol_values = [num_symbols for num_symbols in curves if num_symbols != reference_num_symbols]
    fig, axes = plt.subplots(2, 2, figsize=(7.2, 5.2), sharex=True)
    eps = 1e-300
    for ax, (component, title) in zip(axes.ravel(), PLOT_COMPONENTS, strict=True):
        ref = np.maximum(np.abs(reference[component]), eps)
        for num_symbols in num_symbol_values:
            data = curves[num_symbols]
            relerr = np.abs(data[component] - reference[component]) / ref
            ax.plot(data["llw_grid"], relerr, marker="o", ms=2.5, lw=1.0, label=f"{num_symbols} symbols")
    ...
    axes[0, 0].legend(fontsize=6)
    fig.suptitle(f"Relative to longest pulse window: {reference_num_symbols} symbols")
    fig.tight_layout()
    path = out_dir / f"xhkm_pulse_window_convergence_relative_{pulse_shape}.pdf"
    fig.savefig(path, dpi=300)
    plt.close(fig)
    return path


def plot_walkoff_fraction(curves: dict[int, dict[str, np.ndarray]], out_dir: Path, pulse_shape: str) -> Path:
    fig, ax = plt.subplots(figsize=(5.0, 3.2))
    for num_symbols, data in curves.items():
        ax.plot(data["llw_grid"], data["walkoff_fraction"], marker="o", ms=2.5, lw=1.0, label=f"{num_symbols} symbols")
    ax.axhline(0.5, color="0.35", lw=0.7, ls=":", label="50%")
    ax.axhline(0.8, color="0.35", lw=0.7, ls="--", label="80%")
    ax.set_xscale("log")
    ax.set_xlabel(r"$L/L_W$")
    ax.set_ylabel("walkoff / time window")
    ax.grid(True, which="both", alpha=0.25)
    ax.legend(fontsize=6)
    fig.tight_layout()
    path = out_dir / f"xhkm_pulse_window_walkoff_fraction_{pulse_shape}.pdf"
    fig.savefig(path, dpi=300)
    plt.close(fig)
    return path
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Check xhkm pulse time-window convergence for 2PC, 3PC, and 4PC.")
    ...
    parser.add_argument("--min-pts-per-collision", type=float, default=10.0)
    parser.add_argument("--max-z-points", type=int, default=2000)
    parser.add_argument(
        "--num-symbols",
        type=int,
        nargs="+",
        default=(128, 220, 384, 512),
        help="Pulse time-window lengths to compare. The last value is the reference.",
    )
...
def main() -> None:
    ...
    for num_symbols in args.num_symbols:
        paths[int(num_symbols)] = compute_num_symbols(
            pulse_shape=args.pulse,
            window=args.window,
            num_symbols=int(num_symbols),
            llw_grid=llw_grid,
            min_pts_per_collision=args.min_pts_per_collision,
            max_z_points=args.max_z_points,
            recompute=args.recompute,
        )
        print(f"saved {paths[int(num_symbols)]}")
    ...
    for path in (
        ...
        plot_walkoff_fraction(curves, OUT_MEDIA, args.pulse),
    ):
        print(f"saved {path}")
