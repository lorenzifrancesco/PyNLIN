...
def get_I_low(fiber, wdm, recompute=False):
    nyquist_pulse = NyquistPulse(
        baud_rate=cf.baud_rate,
        num_symbols=220,  # CHANGING THIS SOLVES THE ALIASING PROBLEM TODO
    ...
    for ipulse, pulse in enumerate([gaussian_pulse, nyquist_pulse]):
        ...
        beta2a_values, beta2b_values = np.meshgrid(
            beta2_range, beta2_range, indexing="xy")

        # Compute and/or load I_low
        save_path = f"results/I_low_{PULSE_NAMES[ipulse]}.npy"
        ...
        save_path_interp = f"results/low_dgd_integral/lowdgd_integral_interp_{PULSE_NAMES[ipulse]}__LoverLD_[{lld_range[0]:.1e}-{lld_range[-1]:.1e}]_N{len(beta2_range)}_Nz{len(z)}.npz"
        if recompute or not os.path.exists(save_path_interp):
            os.makedirs(os.path.dirname(save_path_interp), exist_ok=True)
            # generate the grid interpolation
            I_low_values = np.load(save_path)
            interp_func = RegularGridInterpolator(
                (lld_range, beta2_range),
                I_low_values,
                bounds_error=False,
                fill_value=np.nan
            )
            interp_values = interp_func(
                np.stack([beta2a_values.ravel(), beta2b_values.ravel()], axis=-1)
            ).reshape(beta2a_values.shape)
            np.savez(save_path_interp,   
                    lld_range=lld_range,
                    beta2_range=beta2_range,
                    I_low_values=I_low_values,
                    I_low_interp=interp_values)
            print(f"Saved interpolant data to {save_path_interp}")
        ...
        #         (lld_range.ravel(), lld_range.ravel()),
        #         I_low_values.ravel(),
        #         (beta2a_values, beta2b_values),
        #         method='cubic'
        #     )
        #     np.savez(save_path_interp,   
        #              lld_range=lld_range,
        #              beta2_range=beta2_range,
        #              I_low_values=I_low_values,
        #              I_low_interp=interp_func)
        #     print(f"Saved interpolant data to {save_path_interp}")
        
        # test a bit the interpolating function
        interpolant = np.load(save_path_interp)
        # print(interpolant.files)
        lld_range =    interpolant['lld_range']
        beta2_range =  interpolant['beta2_range']
        I_low_values = interpolant['I_low_values']
        interp_func =  interpolant['I_low_interp']
        # testing the interpolating function:
        lg.trace(f"Testing the interpolating function for pulse {PULSE_NAMES[ipulse]}:")
        test_points = [(0, 0), (0, 2), (2, 0), (1, 1), (2, 2)]
        for point in test_points:
            lg.trace(f"Point {point}: Interpolated = {interp_func[point[1], point[0]]}")
        I_low_values_multipulse.append(np.load(save_path))
    return I_low_values_multipulse, lld_range # I_low_values 1: Gaussian, 2: Nyquist
...
def plot_dispersion_analysis(fiber, wdm, recompute=True):
    ...
    for ipulse, I_low_values in enumerate(I_low_values):
        ...
        contour      = plt.contourf(lld_range, lld_range, np.clip(I_low_values, a_min=-10, a_max=0.91), levels=100, cmap='viridis')
        contour_lines = plt.contour(lld_range, lld_range, np.clip(I_low_values, a_min=-10, a_max=0.16), levels=50, colors="w")
        ...
        plt.tight_layout()
        plt.savefig("media/differential_dispersion_"+PULSE_NAMES[ipulse]+".pdf")
        print(
...
if __name__ == "__main__":
    ...
    # Run both plots
    plot_dispersion_analysis(fiber, wdm, recompute=False)
