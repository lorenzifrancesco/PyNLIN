"""Ideal (Raman-free, GVD-free) softplus fit utilities for NLIN lookup curves."""
...
def _ideal_fit_coefficients_cached(
    ...
    assert len(llw_numeric) == len(
        nlin_numeric), f"{pulse_shape}: {len(llw_numeric)} vs {len(nlin_numeric)}"
    popt, _ = curve_fit(softplus,
                        llw_numeric,
                        nlin_numeric * y_norm,
                        p0=p0)
    lg.debug(
        "[ideal_fits] Fitted (a,b,c)=({:.3e},{:.3e},{:.3e}) with x_norm={:.3e}".format(
...
def ideal_fit_coefficients(gvda: float = 0.0,
    ...
    if gvda != 0.0 or gvdb != 0.0:
        raise RuntimeError(
            "Use dispersion correction for nonzero GVD; ideal fit expects gvda=gvdb=0."
        )
...
if __name__ == "__main__":
    raise SystemExit(
        "Use ideal_fit_coefficients(..., fiber_length=..., baud_rate=..., n_samples_numeric_n=...) "
        "from a run-specific config context."
    )
