...
from pynlin.methods.td.collision import MAX_LLD, build_I_low_interpolator
from pynlin.methods.td.estimation.raman_integrals import load_fB
from pynlin.fiber import MMFiber
from pynlin.utils import dBm2watt, watt2dBm

init_logging()

from pynlin.methods.td.estimation.ideal_fits import ideal_fit_coefficients, softplus
from pynlin.methods.td.estimation.lo_correction import (
    build_lookup_integral_table_with_raman,
)
from pynlin.methods.td.estimation.raman_integrals import (
...
def collision_coeffs_system(cf,
    ...
    fiber_km = getattr(cf, "fiber_length", None) or getattr(getattr(cf, "fiber", None), "length", None)
    if fiber_km is None:
        raise ValueError("fiber length not found on config/fiber object")
    filename = s3_pair_nlin_kernel_path(
        ipulse=ipulse,
        fiber_type=fiber_type,
        br_hz=br_hz,
        n_ch=n_ch,
        fiber_length=fiber_km,
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        raise SystemExit(
            "Usage: python -m pynlin.methods.td.legacy_estimator <system.toml> [numerical_config.toml]"
        )
