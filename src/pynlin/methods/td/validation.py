...
def _load_cf_with_legacy_support(
    cf_path: str = "./input/studies.toml",
    ...
    try:
        nc = cfg.load_nc_toml_to_struct(nc_path) if nc_path is not None else getattr(cf, "numerics", None)
        if nc is None:
            raise ValueError("No numerics available")
...
def _numerics_from_config(cf):
    nc = getattr(cf, "numerics", None)
    if nc is None:
        raise ValueError("Validation requires a [numerics] section in the system TOML.")
    return nc
