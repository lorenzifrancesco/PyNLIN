...
def _ideal_fit_coefficients_cached(
    ...
    popt, _ = curve_fit(softplus,
                        ...
                        p0=p0)
    lg.debug(
        "[ideal_fits] Fitted (a,b,c)=({:.3e},{:.3e},{:.3e}) from normalized S1 reference {}".format(
...
if __name__ == "__main__":
    raise SystemExit(
        "Use ideal_fit_coefficients(...) from a run-specific context; the S1 reference is now "
        "loaded from the normalized on-disk dataset."
    )
