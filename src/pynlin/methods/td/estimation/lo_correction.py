...
from pynlin.methods.td.collision import MAX_LLD, build_I_low_interpolator, ensure_i_low_dataset
...
def build_lookup_integral_table_with_raman_custom(cf,
    ...
    if os.path.exists(filename) and not recompute:
        lg.debug(f"Loading precomputed Raman correction grid from {filename}")
        with np.load(filename, allow_pickle=False) as data:
            raman_correction_grid = data["s2b_lo_corr_custom"]
    else:
        lg.debug(f"Computing Raman correction grid and saving to {filename}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        raise SystemExit(
            "Usage: python -m pynlin.methods.td.estimation.lo_correction <system.toml> [numerical_config.toml]"
        )
